...
validation_loader = DataLoader(
    validation_set, batch_size=5, shuffle=True, drop_last=True
)
...
for epoch in range(100):
    ...
    if validation_corrects > best_accuracy:
        print(
            f"New best model, accuracy update: {best_accuracy} -> {validation_corrects}"
        )
        best_accuracy = validation_corrects
        torch.save(
            model.state_dict(), os.path.join(model_files_path, "model_state_dict.pt")
        )
