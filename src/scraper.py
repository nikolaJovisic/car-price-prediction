...
def scrape() -> None:
    ...
    url = (
        "https://www.polovniautomobili.com/auto-oglasi/pretraga?brand=&brand2=&price_from=&price_to=&year_from=&year_to"
    ...
    for i in range(40):
        ...
        driver.find_element(
            By.XPATH, "//i[@class='uk-icon-angle-double-right']"
        ).click()
...
def extract_textual_data(driver: webdriver.Chrome) -> Dict:
    ...
    price = driver.find_element(
        By.XPATH, "//span[contains(@class, 'priceClassified')]"
    ).text
    data["cena"] = int(re.sub(r"[^0-9]+", "", price))

    general_info_keys = preprocess_textual_data(
        driver,
        "//*[contains(text(), 'Opšte informacije')]/parent::div//div[@class='uk-width-1-2']",
    )
    general_info_values = preprocess_textual_data(
        driver,
        "//*[contains(text(), 'Opšte informacije')]/parent::div//div[@class='uk-width-1-2 uk-text-bold']",
    )
    general_info = dict(zip(general_info_keys, general_info_values))
    data["opste informacije"] = general_info

    additional_info_keys = preprocess_textual_data(
        driver,
        "//*[contains(text(), 'Dodatne informacije')]/parent::div//div[@class='uk-width-1-2']",
    )
    additional_info_values = preprocess_textual_data(
        driver,
        "//*[contains(text(), 'Dodatne informacije')]/parent::div//div[@class='uk-width-1-2 uk-text-bold']",
    )
    additional_info = dict(zip(additional_info_keys, additional_info_values))
    data["dodatne informacije"] = additional_info

    data["sigurnost"] = preprocess_textual_data(
        driver,
        "//*[contains(text(), 'Sigurnost')]/parent::div//div[@class='uk-width-medium-1-4 uk-width-1-2 uk-margin-small-bottom']",
    )
    data["oprema"] = preprocess_textual_data(
        driver,
        "//*[contains(text(), 'Oprema')]/parent::div//div[@class='uk-width-medium-1-4 uk-width-1-2 uk-margin-small-bottom']",
    )
    data["stanje"] = preprocess_textual_data(
        driver,
        "//*[contains(text(), 'Stanje')]/parent::div//div[@class='uk-width-medium-1-4 uk-width-1-2 uk-margin-small-bottom']",
    )
    try:
        description = driver.find_element(
            By.XPATH,
            "//*[contains(text(), 'Opis')]/parent::div//div[@class='uk-width-1-1 description-wrapper']",
        )
